...
SIGN_LORE = {
    "Aries": "The Warrior. You are the spark that starts the fire.",
    "Taurus": "The Builder. You are the earth; patient and unmovable.",
    "Gemini": "The Messenger. You are the wind; weaving stories and ideas.",
    "Cancer": "The Protector. You are the tide; deeply intuitive and loyal.",
    "Leo": "The Radiant. You are the sun; warming the room with presence.",
    "Virgo": "The Alchemist. You are the perfectionist; refining the world.",
    "Libra": "The Diplomat. You are the scales; creating balance and harmony.",
    "Scorpio": "The Sorcerer. You are the depths; diving into mysteries.",
    "Sagittarius": "The Philosopher. You are the arrow; hungry for truth.",
    "Capricorn": "The Architect. You are the peak; driven by legacy.",
    "Aquarius": "The Revolutionary. You are the lightning; breaking structures.",
...
def resolve_location(city_name):
    ...
    try:
        geolocator = Nominatim(user_agent="ia_final_fix_v27")
...
# --- THE EPIC STORY ENGINE (RETURNS CHAPTER LIST) ---
def generate_hero_chapters(name, chart_data, orientation_title, lp, struggle_advice):
    ...
    chapters.append({
        "title": "🌟 THE ORIGIN",
        "body": f"Long ago, the universe conspired to create a unique frequency, and it named that frequency {name}. You were born under the blazing sun of {sun_sign}. {sun_lore} This is your core, your fuel, and your inevitable nature.\n\nBut to protect this intense light, you donned a mask. To the world, you first appear as {ris_sign}. {ris_lore} This is your armor and your first impression. Your journey begins by reconciling these two: the inner fire of the {sun_sign} and the outer shield of the {ris_sign}."
    })
...
def create_pdf_b64(name, lp, lp_desc, orientation_title, orientation_body, hero_chapters, advice, chart):
    try:
        ...
        # STORY LOOP (Formatted with Space)
        for chap in hero_chapters:
            # Title
            pdf.set_font("Helvetica", 'B', 14)
            pdf.cell(0, 10, clean_text_for_pdf(chap['title']), 0, 1)
            # Body
            pdf.set_font("Helvetica", '', 11)
            pdf.multi_cell(0, 6, clean_text_for_pdf(chap['body']))
            pdf.ln(5) # Space between chapters
        ...
        for k, v in chart.items():
            ...
            pdf.cell(0, 8, f"{k}: {sign} (Archetype {gate}) - {clean_text_for_pdf(name_txt)}", 0, 1)
            pdf.set_font("Helvetica", '', 10)
            pdf.multi_cell(0, 5, f"{clean_text_for_pdf(gate_story)}")
            pdf.ln(3)
...
async def calculate_chart(request: Request):
    ...
    try:
        ...
        # GENERATE CHAPTERS (List of Dicts)
        hero_chapters = generate_hero_chapters(name, chart_data, orientation_title, lp, (topic, advice_text))
        ...
        lp = 0
        lp_desc = "Unknown"
        hero_chapters = [{"title": "Error", "body": "The legend is clouded. Please try again."}]
        topic, advice_text = ("Unknown", "Unknown")

    pdf_b64 = create_pdf_b64(name, lp, lp_desc, orientation_title, orientation_body, hero_chapters, (topic, advice_text), chart_data)

    # HTML OUTPUT: CARD LAYOUT (GRID)
    chapters_html = ""
    for chap in hero_chapters:
        chapters_html += f"""
        <div class="chapter-card">
            <h3>{chap['title']}</h3>
            <p>{chap['body'].replace(chr(10), '<br>')}</p>
        </div>
        """
    ...
    <style>
        ...
        body {{ font-family: 'Source Sans Pro', sans-serif; padding: 20px; line-height: 1.6; color: #333; background: #f9f9f9; }}
        .header {{ text-align: center; margin-bottom: 30px; }}
        h2 {{ font-family: 'Playfair Display', serif; color: #D4AF37; font-size: 2em; margin-bottom: 5px; }}
        ...
        .chapter-card h3 {{
            color: #2c3e50; margin-top: 0; font-size: 1.1em; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 15px;
        }}
        .chapter-card p {{
            color: #555; font-size: 1em; text-align: left; margin-bottom: 0; line-height: 1.6;
        }}
        ...
        .btn {{ 
            ...
            width: 100%; max-width: 300px; margin: 30px auto; text-align: center;
            box-shadow: 0 4px 15px rgba(0,0,0,0.1); text-decoration: none; font-weight: bold;
        }}
    </style>
    </head>
    <body>
        <div class="header">
            ...
            <p style="color:#777;">Your Epic Biography</p>
        </div>
